...
def test_release_versions_are_consistent() -> None:
    ...
    assert manifest["version"] == "1.0.52"
    assert pyproject["project"]["version"] == manifest["version"]
    assert 'VERSION: Final = "1.0.52"' in const_text
...
def test_governance_documents_exist() -> None:
    ...
    required = (
        ...
        "docs/V1_0_47_WEATHER_EVIDENCE_INGESTION.md",
        "docs/V1_0_48_RECORDER_PAYLOAD_HOTFIX.md",
...
def test_supervised_operational_service_metadata_exists() -> None:
    """The bounded manual service must be explicit and documented."""
    services = (
        ROOT / "custom_components/irrigationos/services.yaml"
    ).read_text(encoding="utf-8")
    assert "run_supervised_operation:" in services
    assert "RUN SUPERVISED OPERATIONAL WATERING" in services
...
def test_v1_0_15_is_monotonic_stable_release_candidate() -> None:
    ...
    assert "Installable Home Assistant release:** v1.0.52" in roadmap
