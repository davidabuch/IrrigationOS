"""Daily JSONL operational evidence export for IrrigationOS."""
...
from .health import HEALTH_LOG_RETENTION_DAYS


class DailyOperationalLog:
    """Write safe one-record-per-line daily operational evidence."""
    ...
    def record(self, recorded_at: datetime, payload: dict[str, Any]) -> bool:
        """Append one safe JSONL record and enforce retention."""
        ...
        try:
            ...
            filename = f"irrigationos_{local_time.date().isoformat()}.jsonl"
            ...
            self.last_error = "daily_operational_log_write_failed"
            self.write_error_count += 1
            return False

    def diagnostics(self) -> dict[str, object]:
        """Return safe operational-log health diagnostics."""

        return {
            "directory": str(self.root),
            "current_file": self.current_file,
            "retention_days": HEALTH_LOG_RETENTION_DAYS,
    ...
    def _cleanup(self, local_date: date) -> bool:
        if self._last_cleanup_date == local_date:
            return self.last_error != "daily_operational_log_retention_failed"
        self._last_cleanup_date = local_date
        oldest_kept = local_date - timedelta(days=HEALTH_LOG_RETENTION_DAYS - 1)
        cleanup_healthy = True
        for path in self.root.glob("irrigationos_????-??-??.jsonl"):
            try:
                file_date = date.fromisoformat(path.stem.removeprefix("irrigationos_"))
            except ValueError:
                continue
            if file_date < oldest_kept:
                try:
                    ...
                    self.last_error = "daily_operational_log_retention_failed"
                    self.write_error_count += 1
